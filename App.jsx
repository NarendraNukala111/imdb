...
function Search() {
  ...
  const handleSearch = (e) => {
    ...
    if (e.target.value.length > 2) {
      fetch(`${API_URL}&s=${e.target.value}`)
        .then((res) => res.json())
          ...
          <div className="col-lg-3 col-md-4 col-sm-6 mb-4" key={movie.imdbID}>
            <div className="card shadow-lg rounded border-0">
              ...
              <div className="card-body text-center">
                ...
                <Link to={`/movie/${movie.imdbID}`} className="btn btn-outline-info">
                  📄 Details
                </Link>
                <button
    ...
    <Router>
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow">
        <div className="container d-flex justify-content-between">
          ...
          <div>
            <Link className="btn btn-outline-light me-2" to="/favorites">
              ❤ Favorites
            </Link>
            <Link className="btn btn-outline-light" to="/watchlist">
              🔖 Watchlist
            </Link>
          </div>
        </div>
